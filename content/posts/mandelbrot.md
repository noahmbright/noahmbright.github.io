...
description: Mandelbrot and Julia set explorers — vanilla JS (functional vs imperative) and WebGL. Zoom, pick Julia constant, references.
layout: mandelbrot
date: '2026-02-15'
draft: false
---
...
recursion relation for getting all subsequent numbers is z_{n + 1} = z_n² + c. This sequence will tell us if c is in the Mandelbrot set.

The magnitude of z_n as n tends to infinity will either be
bounded or unbounded. In principle, it's impossible to compute
this in finite time. In practice, we can pick an upper
...
Below are two canvases that render the Mandelbrot set using 2d
...
Hover over either canvas below to do some zooming, and see how the FPS for
each of the implementations compares. 50 iterations per pixel are performed.

[[[VANILLA_CANVASES]]]

Unfortunately for academic beauty, the inline version seems to perform roughly 50–100% faster. I'd have to imagine this comes from JS allocating all these tiny
...
plane. Then we color the plane according to whether or not
...
on the right canvas to move c around, and then explore its Julia set on
the left.
...
- [Numberphile](https://www.youtube.com/watch?v=FFftmWSzgmk). Highly authoritative casual math reference
- [128 bit fixed point opencl Mandelbrot implementation](https://www.bealto.com/mp-mandelbrot_fp128-opencl.html)
- [3Blue1Brown](https://www.youtube.com/watch?v=LqbZpur38nw). Beyond the Mandelbrot set, with links to more advanced topics
